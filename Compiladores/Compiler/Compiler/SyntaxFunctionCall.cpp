...
void Compiler::SyntaxFunctionCall::CheckSyntax() {
	Token* currentToken;
	...
	if (!m_isReturning) {
		...
		if (currentToken->GetLexem() == "(") {
			...
			if (!currentToken) {
				EofError("Error: Expected Logic Expression, ID, or Constant in Function call in '" + m_scope + "'");
				return;
			}
			if (currentToken->GetLexem() == ")") {
				m_isReturning = true;
		...
		if (currentToken->GetLexem() == ")") {
			...
			if (!currentToken) {
				EofError("Error: Expected ';', ')' or ',' in Function call in'" + m_scope + "'");
				return;
			}
...
void Compiler::SyntaxFunctionCall::EofError(std::string description) {
...
void Compiler::SyntaxFunctionCall::PanicMode() {
	Token* currentToken = m_lexAnalyzer->PeekCurrentToken();

	if (!currentToken) {
		EofError("Error: Unexpected Token in Funcion call in '" + m_scope + "'");
		return;
	}
	...
	if (!currentToken) {
		EofError("Error: Unexpected Token in Function call in '" + m_scope + "'");
		return;
	}
	if (currentToken->GetLexem() == ";") {
...
Compiler::SyntaxFunctionCall::SyntaxFunctionCall(LexAnalyzer* lexic, SyntaxAnalyzer* syntax, std::string scope)
: SyntaxState(lexic, syntax) {
	m_isReturning = false;
	m_scope = scope;
}

Compiler::SyntaxFunctionCall::~SyntaxFunctionCall() {
